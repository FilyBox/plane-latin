...
async function boundedText(response: Response, maxBytes: number): Promise<string> {
  ...
  try {
    while (true) {
      // eslint-disable-next-line no-await-in-loop -- streaming reads are ordered
  ...
  async run(event: WorkflowEvent<Params>, step: WorkflowStep) {
    ...
    try {
      const asset = await step.do("resolve receipt", async () => {
        ...
        return JSON.parse(
          await boundedText(await fetch(endpoint, { headers, signal: AbortSignal.timeout(30000) }), 10000)
        ) as Asset;
      });
      let text: string;
      if (asset.type.includes("xml")) {
        text = await step.do("read xml", async () =>
          boundedText(await fetch(asset.url, { signal: AbortSignal.timeout(60000) }), 1024 * 1024)
        );
        ...
        const id = await step.do(
          ...
          () => startTextDetection(this.env, asset.s3_bucket, asset.s3_key)
        );
        ...
        text = await step.do(
          "extract receipt",
          { timeout: "5 minutes", retries: { limit: 2, delay: "10 seconds" } },
          async () => {
...
export default {
  async fetch(request: Request, env: ExpenseEnv): Promise<Response> {
    ...
    const hashes = await Promise.all(
      [request.headers.get("X-Trigger-Secret") || "", env.CF_WORKER_TRIGGER_SECRET || ""].map((value) =>
        crypto.subtle.digest("SHA-256", encoder.encode(value))
      )
    );
    if (!env.CF_WORKER_TRIGGER_SECRET || !crypto.subtle.timingSafeEqual(hashes[0], hashes[1]))
      return Response.json({ error: "Unauthorized" }, { status: 401 });
    if (request.method === "POST" && new URL(request.url).pathname === "/trigger/cancel") {
      try {
        const { attempt } = JSON.parse(await boundedText(new Response(request.body), 4096)) as { attempt: string };
        if (typeof attempt !== "string" || !UUID.test(attempt))
          return Response.json({ error: "Invalid attempt" }, { status: 400 });
        await (await env.EXPENSE_PIPELINE.get(`expense-${attempt}`)).terminate();
        return Response.json({ status: "cancelled" });
      } catch {
        return Response.json({ error: "Unable to cancel workflow" }, { status: 503 });
      }
    }
    if (request.method !== "POST" || new URL(request.url).pathname !== "/trigger/extract")
      return Response.json({ error: "Not found" }, { status: 404 });
    let params: Params;
    try {
      params = JSON.parse(await boundedText(new Response(request.body), 4096)) as Params;
      if (
        ![params.job_id, params.workspace_id, params.attempt].every(
    ...
    try {
      ...
      try {
        await (await env.EXPENSE_PIPELINE.get(id)).status();
      } catch {
