...
const ExpensesPage = observer(function ExpensesPage({ params }: Route.ComponentProps) {
  ...
  const isExpensesEnabled = isWorkspaceFeatureEnabled(workspaceSlug, "payments");
  ...
  if (areFlagsLoaded && isRoleLoaded && (!isExpensesEnabled || !isAdmin))
    return <Navigate to={`/${workspaceSlug}`} replace />;

  return (
    <>
      <PageHead title="Gastos" />
